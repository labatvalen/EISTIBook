<?php
    ...
    function upload($index,$destination,$maxsize=FALSE,$extensions=FALSE)
    {
        ...
        //Déplacement et ajout dans la BDD
            ...
            $image_sizes = getimagesize($_FILES['fic']['tmp_name']);
            $taille = "$image_sizes[0]"."x"."$image_sizes[1]";
            initImg($taille,$nom,$ext);
            move_uploaded_file($_FILES[$index]['tmp_name'],$destination.$nom);
            return (imagethumb($destination.$nom, $destination.'miniatures/'.$nom, 90));
    }
 
    //Action

        $upload = upload('fic','images/',50000, array('png','gif','jpg','jpeg') );
